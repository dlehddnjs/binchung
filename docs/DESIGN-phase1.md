...
- **상태관리 (jotai)**: `filterAtom`(타입/상태 — 이슈 #9, 지역은 스코프 제외), `selectedStationIdAtom`(이슈 #9), `viewportAtom`, `chargersAtom`(이슈 #8). 지도 인스턴스는 atom에 넣지 않는다 (ref로 관리).
...
### 필터/선택 인터랙션 ✅ 계약 확정(이슈 #9, 2026-07-07)

`filterAtom<{ zcode?: string; chgerType?: "fast"|"slow"; stat?: "waiting"|"charging"|"other" }>({})` — `ChargersFeedParams`의 옵션 필드와 동일한 모양이라 `ChargerMap`에서 `{ bbox: viewport, ...filter }`로 단순 스프레드 합성한다. **지역(zcode) 필터 UI는 이번 이슈 스코프에서 제외** — 코드베이스 어디에도 zcode→시도명 매핑이 없고(§2에 "예: 11=서울" 예시 하나뿐, 공식 목록 아님), 실제 매핑 표를 확보하기 전까지 raw 코드를 사용자에게 노출하는 건 UX상 의미가 없다고 판단해 별도 이슈로 미룬다. `filterAtom`/`ChargersFeedParams`의 `zcode` 필드 자체는 타입에 남겨둔다.

⚠️ **알려진 한계(이번엔 고치지 않음)**: 필터를 바꾸면 `chargersQueryKey`가 바뀌어 TanStack Query 입장에선 캐시가 없는 새 쿼리가 된다 — `useChargersFeed`에 항상 같은 `initialData`(전국 스냅샷)를 넘기고 있어서 이 새 쿼리도 그 스냅샷으로 다시 seed되고, `staleTime`(60초) 동안은 필터링 안 된 데이터가 화면에 잠깐 보일 수 있다(실측: 필터 변경 직후엔 `/api/chargers` 재요청이 없다가, `refetchInterval` 60초가 지나야 새 쿼리 파라미터로 실제 요청이 나감 — 헤드리스 Chrome으로 확인). 이슈 #8의 팬/줌에도 이미 있던 현상이라 이번에 처음 생긴 문제는 아니다. 고치려면 `initialData`를 최초 렌더에만 조건부로 주는 별도 변경이 필요해 스코프 밖으로 둔다.

`selectedStationIdAtom<string | null>(null)` — §6 원 3-atom 목록에는 없었으나, 지도 클릭 선택 상태를 `ChargerMap`(writer)과 상세 시트(reader + 닫기 버튼의 writer) 사이에서 공유하기 위해 추가로 필요해졌다.

**클러스터 클릭 규칙**: `ol/source/Cluster`의 클릭된 feature는 항상 `.get("features")`에 1개 이상의 멤버를 담고 있다. 멤버 전원이 같은 `statId`면(멤버가 1개인 경우 포함, 한 충전소의 여러 충전기가 같은 클러스터로 뭉친 경우도 포함) 그 충전소를 선택(`selectedStationIdAtom` 갱신)하고, 서로 다른 충전소가 섞여 있으면 해당 멤버들의 bounding extent로 지도를 줌인한다. 이 판단은 `resolveClusterClick(members): {type:"select"} | {type:"zoomToExtent"}` 순수 함수로 분리해 테스트하고, `ChargerMap`의 클릭 리스너 자체는 OL 배선이라 테스트하지 않는다(§7 예외, 이슈 #8과 동일 원칙).

**상세 시트 데이터**: `chargersAtom`(충전기 단위 행 배열)에서 `selectedStationIdAtom`으로 필터링해 한 충전소의 충전기 목록을 뽑는다(`groupStationChargers`). `statUpdDt`는 `/api/chargers` 응답을 거치면 JSON 직렬화로 문자열이 되고(최초 RSC prop 전달분만 `Date`), 두 타입이 섞여 들어올 수 있으므로 `formatStatUpdDt`가 `Date | string | null`을 모두 방어적으로 처리한다.
...
- [x] 응답 원문을 `fixtures/`에 저장 → 파서 테스트 픽스처로 사용 (정상/에러/빈결과 3종 + 추가 케이스, 총 6개):
  ...
  - `getChargerStatus-empty.xml` (pageNo 범위 초과, resultCode=00, items 빈 배열 — 이슈 #6)
